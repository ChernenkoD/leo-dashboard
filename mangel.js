...
function renderCard(m){
  ...
  return`<div class="mk-card" id="card-${m.id}" style="border-left-color:${bc}"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px"><div style="flex:1;min-width:0;padding-right:8px"><div style="font-weight:800;font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${m.address||"—"}</div>${m.lage?`<div style="font-size:11px;color:var(--muted)">${m.lage}</div>`:""}</div>${deadlinePill(m)}</div><div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px">${statusBadge(m.mangel_status)}${isNew?`<span class="mk-badge mk-badge--new">✨ Neu</span>`:""}${workflowBadge(m.id)}</div><div style="font-size:11px;color:var(--muted);margin-bottom:6px;line-height:1.7"><div>${m.bauleiter||"—"} · ${m.innendienst||"—"}</div><div>${m.ausfuehrungsbeginn||"—"} → ${m.fertigstellung||"—"}</div>${m.first_seen?`<div>Eingang: ${m.first_seen}</div>`:""}</div>${total?`<div class="progress-wrap"><div class="progress-bar"><div class="progress-fill" style="width:${pct}%"></div></div><span class="progress-label">${done}/${total}</span></div>`:""}${renderPositionen(m)}${renderAssignPanel(m)}<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border)"><span style="font-size:10px;color:var(--muted)">${m.leo_url?`<a href="${m.leo_url}" target="_blank" style="color:var(--accent);text-decoration:none">${m.id}</a>`:m.id}</span></div></div>`;
}
